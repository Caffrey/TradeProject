...
import { Request_MarketDataValidSymbols } from "@/requests/requestMarketData";
import { MARKET_DATA_ROUTES } from "@/data/router";

export default function MarketDataHistoryFilter(
     {
        market,
    }:{
        market:string,
    ...
    const [Symbols, setSymbols] = useState<any[]>([]);
     ...
     useEffect(()=>{
        ...
        async function LoadSymbols(){
            const data = await Request_MarketDataValidSymbols(market);
            setSymbols(data);
   ...
   function formatDateTime(date: Date) 
    ...
    const [filter, setFilter] = useState({
     ...
     function GetTradeData(e : React.FormEvent<HTMLFormElement>){
        ...
        e.preventDefault();
          const url =
        `${MARKET_DATA_ROUTES.MarketData_Analysis.herf}?` +
